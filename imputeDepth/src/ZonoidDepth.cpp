...
static int CompareSR(SortRec a, SortRec b)
   ...
   to the RS tableau. */
{
  ...
  sort(x_sort.begin(), x_sort.end(), CompareSR);
...
double ZonoidDepth(double* x, double** xx, int n, int d){
  vector<TPoint> tmpXX(n);
  TPoint tmpX(d);
  for (int i = 0; i < n; i++){
    tmpXX[i] = TPoint(d);
    for (int j = 0; j < d; j++){
  ...
  int err = 0;
  return ZonoidDepth(tmpXX, tmpX, err);
}
